...
export const isRequestDataset = (model: Pick<BaseModel, 'type'>): model is RequestDataSet => (
  model.type === type
);
...
export function update(requestGroup: RequestDataSet, patch: Partial<RequestDataSet> = {}) {
...
export async function getOrCreateForRequestId(requestId: string) {
  ...
  if (!datasets || !datasets.length) {
    const baseId = `${prefix}_${crypto.createHash('sha1').update(requestId).digest('hex')}`;
    try {
...
export async function duplicate(requestGroup: RequestDataSet, patch: Partial<RequestDataSet> = {}) {
  if (!patch.name) {
