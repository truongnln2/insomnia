...
export class RequestSettingsEditor extends PureComponent<Props, State> {
  _editor: MarkdownEditor | null = null;

  constructor(props: Props) {
    super(props);

    const { request, forceEditMode } = this.props;
  ...
  async _updateRequestSettingBoolean(e: React.SyntheticEvent<HTMLInputElement>) {
    ...
    await models.request.update(this.props.request, {
      [setting]: value,
    });
  }
  ...
  async _handleNameChange(name: string) {
    const { request: originalRequest } = this.props;
  ...
  _renderMoveCopy() {
    const { workspaces, request, workspace } = this.props;
    const { activeWorkspaceIdToCopyTo, justMoved, justCopied } = this.state;
